...
<?			if($arParams['ALX_LOAD_PAGE']=='Y' && $APPLICATION->get_cookie("COLLECTED_FDB_SEND_".$ALX) != 'Y'):?>
...
<?		if ($arParams['ALX_LINK_POPUP'] !== 'Y'):?>
		...
		<div id="collect_err_<?=$ALX?>" class="alx-feedb-error"></div>
<?		endif?>
<?
...
<?		require("form.php"); // include form ?>
		...
		<script type="text/javascript">
			$(document).ready(function(){
				var file_w_<?=$ALX?> = parseInt($("#alx_feed_back_<?=$ALX?> .collect_feedback_poles").width() / 5);

				function str_replace_<?=$ALX?>(search, replace, subject){
					return subject.split(search).join(replace);
				}
<?				for($i = 1;$i <= $k;$i++):?>
				$("#alx_feed_back_<?=$ALX?> #collect_file_input_add<?=$i?>")
...
#collect_err_<?=$ALX?>.alx-feedb-error{
	width:<?=$arParams["WIDTH_FORM"];?>;
}
</style>
<?		endif; ?>
...
<?
	...
	<? if (count($arResult["FORM_ERRORS"]) == 0 && ($arResult["success_" . $ALX] == "yes" || $_REQUEST["success_" . $ALX] == "yes")): ?>
		<div class="collect_success_block<?if ($arParams['ALX_LINK_POPUP'] !== 'Y'):?> _without-popup<?endif;?>">
	...
	<? elseif ($arParams["CHECK_ERROR"] == "Y" && count($arResult["FORM_ERRORS"]) > 0): ?>
		...
		<div class="collect_error_block">
			...
			<div class="collect_error_text"><?=GetMessage('ALX_FILL_INPUTS_MSG');?></div>
		</div>
	<? endif;?>
	<?
	...
	<script type="text/javascript">
		...
		<?if (!empty($arResult["FORM_ERRORS"]["ERROR_FIELD"])):
			...
			foreach($arResult["FIELDS"] as $k=>$v)
			{
				...
				{
					?>ALX_fileError($('#alx_feed_back_<?=$ALX?> #collect_<?=mb_strtolower($v["CODE"])?>'));<?
				}
			}
		?>
		<?endif?>
